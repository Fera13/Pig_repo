...
class testGameFunctionality(unittest.TestCase):
    ...
    def test_view_Highscores_wrongType(self):
        with self.assertRaises(ValueError): hs.view_HighScores("hi")
    ...
    def test_add_Compare_Highscores_wrongName(self):
        with self.assertRaises(ValueError): hs.add_Compare_Highscores([], 2)


    def test_add_Compare_Highscores_wrongscore(self):
        with self.assertRaises(ValueError): hs.add_Compare_Highscores("hi", "world")
    ...
    def test_update_High_Score_wrongType(self):
        with self.assertRaises(ValueError): hs.add_Compare_Highscores(3, "world")
        with self.assertRaises(ValueError): hs.add_Compare_Highscores("eren", [])
